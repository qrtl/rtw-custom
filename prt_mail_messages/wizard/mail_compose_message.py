...
class MailComposer(models.TransientModel):
    ...
    def _referenceable_models_fwd(self):
        return [
            (x.model, x.name)
            for x in self.env["ir.model"].sudo().search([("is_mail_thread", "=", True),
                                                         ("model", "!=",
                                                          "mail.thread")])
        ]
    ...
    def get_record_data(self, values):
        ...
        if not subject:
            if values.get("parent_id"):
                ...
                partner_ids = values.get("partner_ids", list()) + [
                    (4, xid) for xid in parent.partner_ids.filtered(
                        lambda rec: rec.email not in
                        [self.env.user.email, self.env.user.company_id.email]
                    ).ids
                ]
                if (
            ...
            re_prefix = (
                _("Fwd:") if self._context.get("default_wizard_mode", False)
                == "forward" else _("Re:")
            )
