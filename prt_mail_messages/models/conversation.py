...
class Conversation(models.Model):
    ...
    def _compute_subject_display(self):
        ...
        for rec in self.with_context(bin_size=False):
            ...
            if rec.last_message_post:
                message_date = pytz.utc.localize(rec.last_message_post
                                                 ).astimezone(local_tz)
            ...
            for participant in rec.partner_ids:
                participant_text = "{} {}".format(
                    ...
                    ' title="%s" src="data:image/png;base64, %s"/>' % (
                        ...
                        if participant.image_128 else IMAGE_PLACEHOLDER,
                    ),
                )
                ...
                if message.message_type != "notification":
                    ...
                    plain_body = (
                        ...
                        ' style="color:#808080;vertical-align:middle;">%s</p>' % (
                            ...
                            if message.author_id else "",
                            message.author_avatar.decode("utf-8")
                            if message.author_avatar else IMAGE_PLACEHOLDER,
                            message_body,
                        )
                    )
                    break
            ...
            rec.subject_display = TREE_TEMPLATE % (
                ...
                if rec.author_id and rec.author_id.image_128 else IMAGE_PLACEHOLDER,
    ...
    def write(self, vals):
        ...
        ) and not self._context.get("only_conversation", False):
            for rec in self:
    ...
    def partner_by_email(self, email_addresses):
        ...
        for address in email_addresses:
            partner = self.env["res.partner"].search([("email", "=ilike", address)],
                                                     limit=1,
                                                     order="id desc")
            if len(partner) == 1:
                return partner
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if not author_id:
            ...
            author_id = (
                partner.id if partner else self.env["res.partner"].create({
                    "name":
                        partner_name if partner_name and len(partner_name) > 0 else
                ...
                if not partner:
                    partner = self.env["res.partner"].create({
                        ...
                        "category_id":
                            ...
                            .ref("prt_mail_messages.cetmix_conversations_partner_cat")
                            else False,
                    })
                # Check if Partner is internal user AND has access to Conversations.
        ...
        return super(
            ...
            self.with_context(
                ...
                if len(other_partner_ids) > 0 else False,
